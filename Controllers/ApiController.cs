...
{
    var listaFuncionarios = context.Funcionarios.ToList();
    return listaFuncionarios;
}

// POST: api/Funcionarios/CreateFuncionario
[HttpPost("CreateFuncionario")]
public IActionResult CreateFuncionario([FromBody] Funcionario funcionario)
{
    var funcionarioDb = context.Funcionarios.Add(funcionario);
    context.SaveChanges();
    return CreatedAtAction(nameof(CreateFuncionario), new { id = funcionarioDb.Entity.Id }, funcionarioDb.Entity);
}

// PUT: api/Funcionarios/UpdateFuncionario/{id}
[HttpPut("UpdateFuncionario/{id}")]
public IActionResult UpdateFuncionario(int id, [FromBody] Funcionario funcionario)
{
    // Verifica se o ID passado na URL é o mesmo do funcionario no corpo
    if (id != funcionario.Id)
    {
        return BadRequest("ID do funcionario na URL e no corpo não coincidem.");
    }

    var funcionarioDb = context.Funcionarios.Find(id);
    ...
    context.Funcionarios.Update(funcionarioDb);
    context.SaveChanges();
    return NoContent();
}
// DELETE: api/Funcionarios/DeleteFuncionario/{id}
...
public IActionResult DeleteFuncionario(int id)
{
    ...
    context.Funcionarios.Remove(funcionarioDb);
    context.SaveChanges();
    return NoContent();
}
